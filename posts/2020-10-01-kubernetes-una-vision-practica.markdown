...
En mi opinión las operaciones de despliegues deberían ser planificadas e implementadas únicamente por **profesionales expertos en sistemas**.
...
```yaml
      ...
      - name: Build and push
        ...
        with:
          ...
          push: true
          tags: geodesicsoflearning/gol:server-${{ steps.ebranch.outputs.branch }}
```
...
metadata:
  name: gol-main
  annotations:
    description: See https://github.com/josejuan/geodesics-of-learning
---
apiVersion: v1
kind: Pod
metadata:
...
spec:
  containers:
    - name: gol-server
      image: geodesicsoflearning/gol:server-main
      env:
        - name: GOL_ENV
          value: gol-main
      ports:
...
```bash
$ kubectl create secret docker-registry dhcred --docker-username=xxx --docker-password=xxx --docker-email=xxx@gmail.com --docker-server=https://index.docker.io/v1/
```
